...
async function refreshShipmentAction(formData: FormData) {
  ...
  const orderId = String(formData.get("orderId") || "");
  if (!orderId) redirect("/hu-admin?ok=0&msg=Chybi+ID+objednavky");
  const ok = await refreshPplShipment(orderId, "HU");
  revalidatePath("/hu-admin");
  redirect(`/hu-admin?ok=${ok ? "1" : "0"}&msg=${ok ? "PPL+stav+obnoven" : "PPL+refresh+selhal"}`);
}
...
async function cancelShipmentAction(formData: FormData) {
  ...
  revalidatePath("/hu-admin");
  redirect(`/hu-admin?ok=${ok ? "1" : "0"}&msg=${ok ? "PPL+zasilka+stornovana" : "PPL+storno+selhalo"}`);
}
...
async function orderPickupAction(formData: FormData) {
  ...
  const result = await orderPplPickup("HU", note);
  revalidatePath("/hu-admin");
...
async function refreshDpdShipmentAction(formData: FormData) {
  ...
  const orderId = String(formData.get("orderId") || "");
  if (!orderId) redirect("/hu-admin?ok=0&msg=Chybi+ID+objednavky");
  const ok = await refreshDpdShipment(orderId, "HU");
  revalidatePath("/hu-admin");
  redirect(`/hu-admin?ok=${ok ? "1" : "0"}&msg=${ok ? "DPD+stav+obnoven" : "DPD+refresh+selhal"}`);
}
...
async function deleteDpdShipmentAction(formData: FormData) {
  ...
  const orderId = String(formData.get("orderId") || "");
  if (!orderId) redirect("/hu-admin?ok=0&msg=Chybi+ID+objednavky");
  const ok = await deleteDpdShipmentForOrder(orderId, "HU");
  revalidatePath("/hu-admin");
  redirect(`/hu-admin?ok=${ok ? "1" : "0"}&msg=${ok ? "DPD+zasilka+smazana" : "Smazani+DPD+zasilky+selhalo"}`);
}
...
async function regenerateDpdLabelAction(formData: FormData) {
  ...
  const orderId = String(formData.get("orderId") || "");
  if (!orderId) redirect("/hu-admin?ok=0&msg=Chybi+ID+objednavky");
  const path = await regenerateDpdLabelForOrder(orderId, "HU");
  revalidatePath("/hu-admin");
  redirect(`/hu-admin?ok=${path ? "1" : "0"}&msg=${path ? "DPD+stitek+vygenerovan" : "DPD+stitek+nelze+vygenerovat"}`);
}

async function orderDpdPickupAction(formData: FormData) {
  "use server";
  const note = String(formData.get("note") || "");
  const result = await orderDpdPickup("HU", note);
  revalidatePath("/hu-admin");
  redirect(`/hu-admin?ok=${result.ok ? "1" : "0"}&msg=${encodeURIComponent(result.message)}`);
}
...
async function bulkDpdLabelsAction(formData: FormData) {
  ...
  redirect(`/hu-admin?ok=${path ? "1" : "0"}&msg=${path ? "DPD+bulk+stitky+vygenerovany" : "DPD+bulk+stitky+selhaly"}`);
}
