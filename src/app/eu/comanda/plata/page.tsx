...
export default async function EuPaymentPage({
  searchParams,
}: {
  ...
  return (
    <main className="mx-auto max-w-2xl px-6 py-12">
      <div className="rounded-3xl border-2 border-[#0d9488]/30 bg-gradient-to-b from-[#e6f7f4] to-white p-8 shadow-lg">
        <h1 className="text-2xl font-bold text-[#0a2624]">Заказ принят — банковский перевод</h1>
        <p className="mt-2 font-semibold text-[#0f766e]">Номер заказа: {nr}</p>
        <div className="mt-8 rounded-2xl border-2 border-[#0d4f4a]/15 bg-white p-6 text-left">
          <h2 className="font-semibold text-[#0a2624]">Реквизиты для оплаты</h2>
          <p className="mt-2 text-[#1a4d47]"><strong>Получатель:</strong> {bank.accountName}</p>
          ...
          <p className="mt-2 text-[#1a4d47]">
            <strong>Сумма:</strong> {order ? `${order.totalPrice} EUR` : "-"}
          </p>
          <p className="mt-2 text-[#1a4d47]">
            <strong>Назначение:</strong> {proforma ? proforma.variable_symbol : `Заказ ${nr}`}
          </p>
          {proforma ? (
            <p className="mt-2 text-sm text-[#1a4d47]">
              <strong>Счёт:</strong> {proforma.invoice_no}
            </p>
          ) : (
            <p className="mt-2 text-sm text-[#9a3412]">
              Счёт готовится. Если письмо не придёт в течение 1–2 минут, напишите нам.
            </p>
          )}
        </div>
        <Link href="/eu" className="mt-8 inline-block rounded-xl bg-[#0d9488] px-8 py-3 font-semibold text-white no-underline hover:bg-[#0f766e]">
          На главную
        </Link>
      </div>
    </main>
